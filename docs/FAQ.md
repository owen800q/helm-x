...
A: 默认不会。proxy 会对连接失败、空响应及所有 HTTP 408/429/5xx 额外重试 10 次（最多总计 11 次请求），每次固定等待 3 秒。Web 控制台 → 服务 → 上游请求重试可修改次数和间隔；`0` 表示无限重试。也可用 `helmx proxy --max-retries 0 --retry-delay 3` 只为当前进程启用无限重试，或用 `--no-retry` 临时关闭。固定间隔不采用上游 `Retry-After`。Ctrl+C 或关闭窗口会立即停止等待中的重试。
