...
UpstreamAttempt upstream_post_once(const std::string& path, const std::string& body,
    ...
    // Same identity passthrough as the WinHTTP branch: relay the codex
    // session/turn headers, and fill in Originator when the client omits it.
    ...
    int posix_status = 0;
    std::string posix_resp;
    if (!http_post(r, posix_status, posix_resp)) {
        result.failure_stage = "transport";
        return result;
    }
    result.status = posix_status;
    result.body = std::move(posix_resp);
...
void handle_client(SOCKET client) {
    ...
    // Prompt mode comes from the same configuration as the rewriter. The
    // loader caches on mtime, so re-reading per request stays cheap while UI
    // edits still take effect without a restart.
    ...
    } else if (prompt_mode == "gpt6") {
        agents = get_resource(ResId::AgentsGpt6);
        log_info("proxy: using gpt6 prompt (gpt-6-instruct / gpt-6-astra)");
    } else {
